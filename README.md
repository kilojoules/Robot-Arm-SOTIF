...
- **Spearman ρ (severity ranking):** We run episodes at 5 budget levels (10%, 30%, 50%, 70%, 90%) and ask: does the monitor's mean predicted P(failure) track how often the policy actually fails as severity increases? Spearman ρ is the rank correlation between the **empirical failure rate** (1 − measured success rate) and the **mean predicted P(failure)** across the 5 budget levels — budget indexes the conditions being correlated, it is not itself one of the correlated variables. ρ = 1.0 means the monitor perfectly ranks "more corruption = more danger" even for a corruption type it has never seen. ρ = 0 means no relationship. ρ < 0 means the monitor gets it backwards. The p-value tests whether the correlation is statistically significant (p < 0.05). Note ρ is computed over only n=5 condition points, while AUROC (below) is over all n=50 episodes — AUROC is the statistically stronger metric.
